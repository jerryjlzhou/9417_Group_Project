...
#       --output AirQualityUCI_feature_engineering.csv \
#       --drop-na-final
#
# Note:
...
def add_future_targets(df: pd.DataFrame, co_low: float, co_high: float) -> pd.DataFrame:
    """
    Create regression and classification targets for future horizons.
    co_low / co_high define the bin boundaries for CO(GT) discretisation.
    """
    df = df.copy()

    # Regression targets for all pollutants
    for col in safe_cols(df, POLLUTANTS):
        for h in HORIZONS:
            df[f"{col}_t+{h}"] = df[col].shift(-h)

    # CO classification bins using dynamic thresholds
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ---- NEW: dynamic CO bin thresholds (quantile-based) ----
    if "CO(GT)" in df.columns:
        co_series = df["CO(GT)"].dropna()
        if len(co_series) == 0:
            raise ValueError("CO(GT) column is present but all values are NaN after cleaning.")
        co_low = co_series.quantile(0.33)
        co_high = co_series.quantile(0.66)
        print(f"[INFO] Dynamic CO bin thresholds: CO_LOW={co_low:.3f}, CO_HIGH={co_high:.3f}")
    else:
        raise ValueError("CO(GT) column not found; cannot create CO classification bins.")
    # ---------------------------------------------------------
    ...
    df = add_rolling_features(df, safe_cols(df, POLLUTANTS + METEOROLOGICAL_VAR))

    # Interaction / residual features
    df = add_interaction_and_residuals(df)

    # Targets (regression + classification)
    df = add_future_targets(df, co_low=co_low, co_high=co_high)
